...
Token *readConstChar(void)
{
  ...
  if (currentChar != EOF && charCodes[currentChar] == CHAR_LETTER)
  {
    ...
    if (currentChar != EOF && charCodes[currentChar] == CHAR_SINGLEQUOTE)
    {
      ...
      error(ERR_INVALIDCHARCONSTANT, ll, cl);
  }
  else
    error(ERR_INVALIDCHARCONSTANT, ll, cl);
...
Token *getToken(void)
{
  ...
  case CHAR_SLASH:
    ...
    {
      // Đây là comment đến cuối dòng
      while (currentChar != EOF && currentChar != '\n')
      {
        readChar(); // Bỏ qua toàn bộ comment
      }
  ...
  case CHAR_GT:
    ...
    return token;
  case CHAR_PERCENT: // Add CHAR_PERCENT
